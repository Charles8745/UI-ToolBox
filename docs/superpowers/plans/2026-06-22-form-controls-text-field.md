...
- 顏色/圓角/字型用既有 token,不硬編。token 值:`--lg-radius-m`=20px(`.lg` 基底已給,不重設)、`--lg-accent`、`--lg-down`=#d2503e、`--lg-tint`、`--lg-shadow`、`--lg-text`、`--lg-text-dim`、`--lg-font`、`--lg-speed`=0.34s、`--lg-ease-out`=cubic-bezier(0.22,0.8,0.3,1)。
- **邊框/陰影/圓角沿用 `.lg` 基底**:`.lg` 已給 `box-shadow:var(--lg-shadow)` 與 `border-radius`,`.lg::before` 已給 rim + `inset 1px stroke`。`.lg-field__box` **不重設 base box-shadow / 不重畫 inset stroke**(否則蓋掉玻璃陰影、描邊變雙層);focus/error ring 必須 `box-shadow: var(--lg-shadow), 0 0 0 2px <色>` **保留** `var(--lg-shadow)`。hover 不另寫(`.lg:hover::after` sheen 已負責)。`.lg::before/::after` 為 `pointer-events:none`,清除鈕在框內可點擊。
...
.lg-field__box {
  ...
  min-height: 58px;
  padding: 0 14px;
  /* 邊框/陰影/圓角沿用 .lg 基底:.lg 已給 border-radius:var(--lg-radius-m)
     + box-shadow:var(--lg-shadow);.lg::before 已給 inset 1px stroke 與 rim。
     此處只加 transition 供 focus/error ring 動畫;hover 回饋由 .lg:hover::after 既有 sheen 負責。 */
...
.lg-field--error .lg-field__box:focus-within {
  box-shadow: var(--lg-shadow), 0 0 0 2px var(--lg-down);
}
...
**plan self-review 修正(已套用):** spec 原寫 focus/error ring 為 `box-shadow: var(--lg-shadow-soft), inset 0 0 0 1px var(--lg-stroke-soft), 0 0 0 2px <色>`,但查證 `.lg` 基底已給 `box-shadow:var(--lg-shadow)`、`.lg::before` 已給 `inset 1px stroke`——照原樣會蓋掉玻璃陰影且描邊雙層。已改為 ring 保留 `var(--lg-shadow)`、不重畫 inset stroke、不另寫 hover(sheen 既有)。spec 對應行已同步修正。
