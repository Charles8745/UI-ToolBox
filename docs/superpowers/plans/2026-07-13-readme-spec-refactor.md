...
另外,「實戰案例(patterns 層)」節(CLAUDE.md:98)的「其 §5 是可接在 AI 規格書之後使用的補充規格」改為「其 §5 是可接在 SPEC.md 之後使用的補充規格」。

- [ ] **Step 6: 驗證與 commit**

```bash
! grep -q "AI 使用規格\|AI 規格書" CLAUDE.md && grep -c "SPEC.md" CLAUDE.md && ! grep -q "18 件元件" CLAUDE.md && echo OK
```
...
- 二輪 review 修正三處:(1) SPEC.md 屬性表 `data-lg-scroll-edge` cell 原含裸管線會切斷 GFM 表格欄位,改為「值 `top` / `bottom` / `both`」寫法;(2) Task 3 補漏 CLAUDE.md:98「可接在 AI 規格書之後」改指 SPEC.md,驗證 grep 加 `AI 規格書` pattern;(3) Task 6 HANDOFF 更新改為「先搬(原括號內文移至 iMarine 段首引文)後改(整行替換)」的確定性指示。
- 二輪 review 另行查證通過:SPEC.md 內容與 CLAUDE.md 規格塊逐元件比對等價(26 件、屬性、API、材質、tokens 無增減);規格引用的 7 個圖示(x/check/minus/plus/star-fill/cloud-arrow-up/trend-up)皆在 icons.json;site.src.html 錨點(2028/2077/2079/2106)與 build_site.py 修改點核實;leftover 正則涵蓋 `{{AI_SPEC}}`;build 失敗時在寫檔前 exit,不會產出壞的 index.html;掃描 pattern 不會誤傷莫內「十八幅」與 revive 延遲值 1800。
